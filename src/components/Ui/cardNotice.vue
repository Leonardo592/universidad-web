<template>
  <article @click="$emit('click')" class="group cursor-pointer flex flex-col h-full bg-white shadow-lg transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 overflow-hidden border-b-4 border-transparent hover:border-uancv-red">
    ...
    <div class="p-6 flex flex-col flex-grow">
      ...
      <h3 class="text-xl font-bold text-uancv-blue-dark leading-tight line-clamp-2 group-hover:text-uancv-red transition-colors">
        {{ titulo }}
      </h3>
      <p v-if="contenido" class="mt-3 text-sm text-uancv-text-secondary line-clamp-3 flex-grow">
        {{ contenido }}
      </p>
